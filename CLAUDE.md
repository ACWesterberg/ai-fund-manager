...
- **The position cap counts a run's own buys, and the mandate's prose never outranks it.** `max_positions` used to be checked against a set computed before the verdict loop and never grown, so it only fired when the book was *already* at the limit — it never once limited how many names a run opens. A clean-slate what-if starts at zero positions, so the cap was inert there entirely; a live book holding 8 against a cap of 10 would approve five new buys and end at 13. Only the prompt's "Max open positions: 10" was holding the count down, and prose is not a guardrail. The projected set now grows as buys are approved and shrinks on a full exit, so "sell A, open B" still fits a full book while a partial trim does not free a slot. Arrival order decides the last slot, matching the sector and cash checks in the same pass. Separately: **the cap is not a function of capital** — every profile ships 10 whether it holds 50,000 SEK or 2,000,000, so a large run comes out concentrated unless you say otherwise. `config.OVERRIDABLE_RISK` is the per-run knob (What-If Lab and sleeve review both take it), and `risk_override_note` tells the model which figure wins, because every mandate restates its caps in prose and nothing keeps that text in sync with `RiskConfig`.
...
- **Risk caps per run.** `max_positions` and `max_position_pct` are overridable from the form. The position cap is the number that quietly decides how concentrated a large amount comes out — the form shows what one name is worth at the cap before the call is spent, and the result's Positions KPI names the cap alongside the count. Note the mandates carry a conviction-weight table (10–15% high / 5–9% medium / 3–5% starter) that independently implies about ten names: raising the cap tells the model the new figure wins, but a much wider book needs that table loosened too, and that is investment policy rather than config.
...
| `max_positions` | 10 | counted across the whole run, not just what was held before it; **does not scale with capital** |
...
src/fundmgr/config.py         AppConfig + all sub-configs; get_enabled_tickers, load_universe;
                              OVERRIDABLE_RISK + clean_risk/apply_risk_overrides (shared per-run caps)
